...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if (curStage == "ejected")
		{
			...
			if (!tweeningChar && !inCutscene)
			{
				...
				FlxTween.tween(gf, {x: FlxG.random.float(gfStartpos.x - 10, gfStartpos.x + 10), y: FlxG.random.float(gfStartpos.y - 10, gfStartpos.y + 10)},
					0.4, {
					ease: FlxEase.smoothStepInOut
				});
				FlxTween.tween(dad, {x: FlxG.random.float(dadStartpos.x - 15, dadStartpos.x + 15),
					y: FlxG.random.float(dadStartpos.y - 15, dadStartpos.y + 15)},
					0.4, {
						ease: FlxEase.smoothStepInOut
					});
			}
	...
	function jumpscare(chance:Float, duration:Float)
	{
		...
		if (outOfTen <= ((!Math.isNaN(chance)) ? chance : 4))
		{
			...
			camHUD.shake(0.0125 * (jumpscareSizeInterval / 2), (((!Math.isNaN(duration)) ? duration : 1) * Conductor.stepCrochet) / 1000, function()
			{
				jumpScare.visible = false;
		...
		for (i in 0...achievementsToCheck.length)
		{
			...
			if (!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled && !unlock)
			{
				switch (achievementName)
				{
					...
					case 'mira-hq_week':
						if (ClientPrefs.SussusToogusSongComplete
							&& ClientPrefs.LightsDownSongComplete
							&& ClientPrefs.ReactorSongComplete
							&& ClientPrefs.EjectedSongComplete)
						{
							unlock = true;
						}
					...
					case 'airship_week':
						if (ClientPrefs.MandoSongComplete && ClientPrefs.DLowSongComplete && ClientPrefs.OversightSongComplete
							&& ClientPrefs.InfluenceSongComplete && ClientPrefs.DangerSongComplete && ClientPrefs.DoubleKillSongComplete
							&& ClientPrefs.DeathBlowSongComplete)
						{
							unlock = true;
						}

					case 'tt-gray_week':
						if (ClientPrefs.InsaneSongComplete && ClientPrefs.BlackoutSongComplete && ClientPrefs.NyctophobiaSongComplete
							&& ClientPrefs.MassacreSongComplete)
						{
							unlock = true;
						}
