...
class SusSubState extends MusicBeatSubstate
{
	...
	var words:Array<String> = [
		'SUS', 'OISE', 'LEAVE HURRY', 'HE DIED', 'DYING', 'INSANE', 'REACTOR', 'KILL', 'ZIFFY', 'DONT BELONG', 'ABANDONED', 'BOO!', 'NOT WANTED', 'TIRESOME',
		'USELESS', 'GRUESOME', 'IMPOSTOR', 'GET OUT', 'HOPELESS', 'RUN', 'CLOWN', 'CAN YOU SEE?', 'WHERE?', 'AIRSHIP', 'HELP', 'XXXXX', 'AMOGUS', 'SUSSY',
		'IT FAILED', 'I SEE YOU', 'POLUS'
	];
	var onePercentWords:Array<String> = ['YELLOW', 'KILLED', 'PINK SUS LOL', 'RED', 'GRAY', "RUN", 'BLACK', 'GREEN'];
	var zeropointonePercentWords:Array<String> = ['YOU ARE DEAD', 'THIS IS FANMADE', 'AMONG US', 'MAKED BY MERPHI'];
	...
	{
		if (letter == realWord.charAt(position) || letter.toLowerCase() == realWord.charAt(position).toLowerCase())
			correctLetter();
		else
